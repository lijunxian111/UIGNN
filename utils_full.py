...
def load_beijing_data(build_adj=True, pred_attr = 'PM25_Concentration'):
    aq_location_path = 'data/Beijing/stations.csv'
    data_path = 'data/Beijing/processed_raw.csv'
    meta_path = 'data/Beijing/meta_data.pkl'
    ...
    if build_adj:
        ...
        for t in range(num_station):
            for c in range(num_station):
                dis = haversine(beijing_location.at[t, 'longitude'],
    ...
    drop_attrs = ['PM25_Concentration', 'PM10_Concentration','NO2_Concentration', 'CO_Concentration', 'O3_Concentration', 'SO2_Concentration']
    drop_attrs += ['PM25_Missing','PM10_Missing', 'NO2_Missing', 'CO_Missing', 'O3_Missing', 'SO2_Missing']
    ...
    for id, station_aq in beijing_multimodal.groupby('station_id'):
        ...
        #data['feat'].append(station_aq.drop(columns=drop_attrs).to_numpy()[np.newaxis])
        data['feat'].append(station_aq[pred_attr].to_numpy()[np.newaxis])

    data['feat'] = np.concatenate(data['feat'], axis=0)[:, :]
    X = data['feat']
    data['time'] = station_aq.index.values.astype(np.datetime64)
...
def test_error_forecasting(STmodel, unknow_set, test_data, A_s, E_maxvalue, Missing0, mc_drop, EDL, quantile):
    """
    :param STmodel: The graph neural networks
    :unknow_set: The unknow locations for spatial prediction
    :test_data: The true value test_data of shape (test_num_timesteps, num_nodes)
    ...
    random_num = random.randint(0,test_data.shape[0]-6-time_dim-1)
    ...
    for i in range(0, test_data.shape[0]-6-time_dim):
        ...
        if mc_drop:
            if i == random_num:
                A_shape = A_q.shape[0]
                print(i)
                forecasting = np.zeros((1, A_shape), dtype=np.float32)
                for _ in range(num_samples):
                    forecasting_once, _, _ = STmodel(T_inputs, A_q, A_h)
                ...
                forecasting, _, _ = STmodel(T_inputs, A_q, A_h)
                forecasting = forecasting.data.numpy()
                pred[i, :] = forecasting[0, :]

        elif EDL:
            #print(T_inputs.shape)
            forecasting, out_prob, _ = STmodel(T_inputs, A_q, A_h)
            forecasting = forecasting.data.numpy()
            pred[i, :] = forecasting[0, :]
    ...
    if quantile:
        ...
        mis_std_all = np.std(mis_pred_, axis=0)
        obs_std_all = np.std(pred[:,obs_idx], axis=0)

        #mis_uncertaintys = np.sum(mis_std_all)/np.sum(mis_test_mask_)
        mis_uncertaintys = np.mean(mis_std_all)
        #obs_uncertaintys = np.sum(obs_std_all)/np.sum(test_mask[:,obs_idx])
        obs_uncertaintys = np.mean(obs_std_all)
    if EDL:
        obs_idx = list(set(range(0, test_inputs_s.shape[1])) - unknow_set)
    ...
    """
    if quantile:
        mis_var_all = np.var(mis_pred_, axis=0)
        obs_var_all = np.var(pred[:, obs_idx], axis=0)
        
        NLL_obs = G_NLL(obs_target_, np.mean(obs_pred_, axis=0), obs_var_all)
        NLL_mis = G_NLL(mis_target_, np.mean(mis_pred_, axis=0), mis_var_all)
    """
    # MAPE = np.sum(np.abs(o - truth)/(truth + 1e-5))/np.sum( test_mask)
    ...
    if quantile:
        return [MAE_obs, RMSE_obs, R2_obs, obs_uncertaintys], [MAE_mis, RMSE_mis, R2_mis,mis_uncertaintys], [MAE, RMSE, R2]
    else:
...
def G_NLL(y:np.ndarray,me:np.ndarray,va:np.ndarray):
    variance2 = np.log(np.exp(va)+1.0)+0.001
    nll = (np.log(variance2)-2.0 + (y - me)*(y - me) - 2 * variance2).mean()
    return nll
