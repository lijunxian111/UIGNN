...
from utils_full import load_metr_la_rdata, get_normalized_adj, get_Laplace, calculate_random_walk_matrix,test_error,test_error_forecasting,load_nerl_data,load_sedata,load_udata,load_pems_data,load_beijing_data
import random
import pandas as pd
from structure_full import D_GCN, C_GCN, K_GCN,IGNNK
from EDL_loss import EvidentialLossSumOfSquares
from quantile_reg import quantile_loss
...
if __name__=="__main__":
    ...
    for epoch in range(Max_episode):
        ...
        for i in range(
            ...
            #know_set= torch.from_numpy(np.array(list(know_set))).long()

            optimizer.zero_grad()
            if EDL:
                X_res, out_prob, X_rec = STmodel(Mf_inputs, A_q, A_h)
                loss1 = criterion(out_prob * mask, outputs * mask)
                loss2 = rec_loss(X_rec,Mf_inputs)
            else:
                X_res,_,X_rec = STmodel(Mf_inputs, A_q, A_h)  # Obtain the reconstruction

                loss1 = criterion(X_res * mask, outputs * mask)
                loss2 = rec_loss(X_rec, Mf_inputs)
        ...
        if epoch % 10 == 0:
            ...
            obs, mis, overall = test_error_forecasting(STmodel, unknow_set, test_set, A, E_maxvalue, True, mc_drop=MC_dropout, EDL=EDL, quantile=quantile)
            ...
            if quantile:
              mis_std_list.append(mis[3])
              obs_std_list.append(obs[3])
        ...
        if epoch % 10 == 0:
            ...
            if quantile:
                print("mis_uncer",epoch, mis[3])
                print("obs_uncer",epoch, obs[3])
    ...
    if quantile:
        print("final uncer: ")
        print(mis_std_list[-1].mean())
        print(obs_std_list[-1].mean())


    best_epoch = np.argmin(np.array(RMSE_obs_list))
    print('RMSE Best model result:', np.array(RMSE_list)[best_epoch], np.array(RMSE_obs_list)[best_epoch], np.array(RMSE_mis_list)[best_epoch])
    ...
    if quantile:
        print('STD Best model result:', np.array(obs_std_list)[best_epoch], np.array(mis_std_list)[best_epoch])
