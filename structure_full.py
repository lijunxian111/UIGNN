...
class GAT(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        a_input = torch.cat([h.repeat(1, 1, N).view(B, N * N, self.in_channels), h.repeat(1, N, 1)], dim=2).view(B, N,
                                                                                                                 N,
                                                                                                                 2 * self.in_channels)
        e = self.leakyrelu(torch.matmul(a_input, self.a).squeeze(3))
        zero_vec = -9e15 * torch.ones_like(e)

        attention = torch.where(adj.unsqueeze(0).repeat(B, 1, 1) > self.threshold, e,
...
class IGNNK(nn.Module):
  ...
  x^y_m      v
    ...
    def __init__(self, h, z, k, forecasting=False, mc_dropout=False, EDL=False):
        ...
        # change last layer output to 1
        self.GNN3 = D_GCN(self.hidden_dimension, self.time_dimension, self.order, activation='linear')
        if forecasting:
            self.lin_out = nn.Linear(self.time_dimension, 1)
        if EDL:
            self.lin_prob = nn.Linear(self.time_dimension, 3)
        ...
        if mc_dropout:
            ...
            self.MC_dropout = F.dropout

    def uncertainity_estimate(self, outputs):
        y_mean = torch.mean(outputs, dim=-1)
        y_variance = torch.var(outputs, dim=-1)
        tau = 0.02 * (1. - self.dropout_rate) / (2. * outputs.shape[-1] * self.decay)
        # print(tau)
        y_variance += (1. / tau)
        y_std = torch.std(outputs, dim=-1)
        return y_mean, y_std
    ...
    def forward(self, X, A_q, A_h):
        ...
        if self.mc_dropout:
            X_s1 = self.MC_dropout(X_s1,training=True, p=self.dropout_rate)
        X_s2 = self.GNN2(X_s1, A_q, A_h) + X_s1  # num_nodes, rank
        if self.mc_dropout:
            X_s2 = self.MC_dropout(X_s2,training=True, p=self.dropout_rate)
        X_s3 = self.GNN3(X_s2, A_q, A_h)
        """
        if self.EDL:
            X_prob = torch.abs(self.GNN_prob(X_s2, A_q, A_h)).permute(0, 2, 1)
        """

        #X_res = X_s3.permute(0, 2, 1)
        X_res = self.lin_out(X_s3)
        X_res = X_res.permute(0, 2, 1)
        #X_s3 = X_s3.permute(0, 2, 1)
        if self.mc_dropout:
            X_s3 = X_s3.permute(0, 2, 1)
            # X_mean,X_std=self.uncertainity_estimate(X_res)
            return X_res, None, X_s3
        if self.EDL:
            X_prob = self.lin_prob(X_s3)
            X_prob = torch.abs(X_prob.permute(0, 2, 1))
            out = torch.cat((X_res, X_prob), dim=1)
            X_s3 = X_s3.permute(0, 2, 1)
            return X_res, out, X_s3
        X_s3 = X_s3.permute(0, 2, 1)
        return X_res, None, X_s3
