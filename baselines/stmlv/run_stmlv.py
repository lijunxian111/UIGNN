...
if __name__ == "__main__":
    ...
    norm = np.sum(adj, axis=0)

    norm[norm == 0] = 1

    # adj[:, list(unknow_set)] = 0
    adj = adj / norm
    tmp_train = np.matmul(knn_training_extended, adj)
    knn_training_extended[:, list(unknow_set)] = tmp_train[:, list(unknow_set)]
    tmp_test = np.matmul(knn_test_extended, adj)
    knn_test_extended[:, list(unknow_set)] = tmp_test[:, list(unknow_set)]
    ...
    training_mask = np.ones(knn_training_original.shape)
    training_mask[knn_training_original == 0] = 0
    test_omask = np.ones(knn_test_original.shape)
    test_omask[knn_test_original == 0] = 0
    print('recovered rmse')
    print('training mis: ' + str(np.sqrt(
        (np.sum(np.square(knn_training_extended - knn_training_original)[:, list(know_set)])) / (
            np.sum(training_mask[:, list(know_set)])))))
    print('training mis: ' + str(np.sqrt(
        (np.sum(np.square(knn_training_extended - knn_training_original)[:, list(unknow_set)])) / (
            np.sum(training_mask[:, list(unknow_set)])))))
    print('test obs: ' + str(np.sqrt((np.sum(np.square(knn_test_extended - knn_test_original)[:, list(know_set)])) / (
        np.sum(test_omask[:, list(know_set)])))))
    print('test mis: ' + str(np.sqrt((np.sum(np.square(knn_test_extended - knn_test_original)[:, list(unknow_set)])) / (
    ...
    for epoch in range(Max_episode):
      ...
      for i in range(
        ...
        t_random = np.random.randint(0, high=(knn_training_extended.shape[0] - h - SLICE - 1), size=batch_size,
                                     dtype='l')
        feed_batch = []
        target_batch = []
        for j in range(batch_size):
            ...
            target_batch.append(knn_training_extended[t_random[j] + h + SLICE: t_random[j] + h + SLICE + 1,
